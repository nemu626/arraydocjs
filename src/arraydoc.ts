import { DefaultDocumentStyle, BorderChar } from "./constant";
...
export const drawCharLine = (start: string, pad: string, padLength: number, contents: string[], interBorder: string, end: string): string => {
  const padString = pad.repeat(padLength);
  const result = 
    start 
    + contents.map((c, i) => `${padString}${c}${padString}${i !== contents.length - 1 ? interBorder : ''}`).join('')
    + end
  return result;
}

export const drawSegment = (segment: Segment, position: SegmentPosition, cellLength: number, arrayLength: number): string => {
  ...
  const startChar = position === 'top' ? BorderChar.leftTop[lineStyle]: BorderChar.leftBottom[lineStyle];
  const endChar = position === 'top' ? BorderChar.rightTop[lineStyle]: BorderChar.rightBottom[lineStyle];
  const contentPointChar = position === 'top' ? BorderChar.bottomT[lineStyle]: BorderChar.topT[lineStyle];
...
export const array2Doc = <T>(document: ArrayDocument<T>): string => {
  ...
  const stringArray = typeof document.body === 'string' 
    ? document.body.split('') 
    : document.body.map(accessor);

  const parsedStrs: string[] = 
    stringArray.map(str => style.charLength ? str.substring(0, style.charLength) : str)
  ...
  const topSegmentStrs = document.segments?.top.map(segment => drawSegment(segment, 'top', cellLength, document.body.length)) || []
  const bottomSegmentLine = document.segments?.bottom.map(segment => drawSegment(segment, 'bottom', cellLength, document.body.length)) || []
